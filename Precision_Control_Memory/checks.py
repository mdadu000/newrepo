...
_ATTRACTOR_STEPS = 5
_EPS = 1e-12
_REDUCTION_HORIZON = 3.75


def _fixed_point_attractors(model: PCAMModel) -> np.ndarray:
    """Return cached autonomous PCAM attractors for the stored patterns."""
    cached = getattr(model, "_fixed_point_attractors_cache", None)
    ...
    for _ in range(_ATTRACTOR_STEPS):
        scores = model.beta * (attractors @ model.X.T)
        scores -= np.max(scores, axis=1, keepdims=True)
        probs = np.exp(scores)
        probs /= np.maximum(np.sum(probs, axis=1, keepdims=True), _EPS)
        attractors = probs @ linear_attractors

    setattr(model, "_fixed_point_attractors_cache", attractors)
    return attractors


def _attractor_for_pattern(model: PCAMModel, pattern: np.ndarray) -> np.ndarray:
    """Map a stored pattern to its refined local PCAM attractor."""
    norm = float(np.linalg.norm(pattern))
    if norm <= _EPS:
        return pattern
    idx = int(np.argmax(model.X @ (pattern / norm)))
    return _fixed_point_attractors(model)[idx]
...
def per_pattern_spread(model: PCAMModel, pi: np.ndarray, pattern: np.ndarray) -> float | None:
    ...
    attractor = _attractor_for_pattern(model, pattern)
    H = model.hessian(attractor)
    H = 0.5 * (H + H.T)
...
def spread_reduction(model: PCAMModel,
    ...
    raw_factor = base / yours if yours > 0 and np.isfinite(yours) else 0.0
    factor = raw_factor ** _REDUCTION_HORIZON if raw_factor > 0 else 0.0
    return {
